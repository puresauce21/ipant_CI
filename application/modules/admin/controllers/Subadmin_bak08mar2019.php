    ...
    public function save(){
		if($this->input->post()){
			...
			if($this->form_validation->run() === false){   
				...
				$this->Common_model->addDataIntoTable("Users",$data);
...
*/
    ...
    */
    ...
    public function getsubadminDetail($id=''){ 
        /* breadcrumb code start */
        $this->breadcrumbs->push('<i class="fa fa-dashboard"></i> '.$this->lang->line('dashboard') , 'admin/dashboard');
        $this->breadcrumbs->push($this->lang->line('subadmin_list'), 'admin/subadmin');
        $this->breadcrumbs->push($this->lang->line('edit_subadmin'), '/');
        /*breadcrumb code end */
        if(!empty($id)){
            $data['roleDetails'] =  $this->Common_model->getDataFromTabel('Roles', '*');
            $data['title']=$this->lang->line('edit_subadmin');
            $data['button']=$this->lang->line('update'); 
            $subadminDetails =  $this->Common_model->getDataFromTabel('Users', '*', array( 'id'=> decode($id), 'role_id' => 5));
    ...
    */  
    public function subadminDelete(){
        ...
        $whereCondtion  = array('id'=>$id, 'role_id'=>5 );
    ...
    public function deleteSubAdmin(){
        ...
        if(!empty($deleteIdArray)){
           foreach($deleteIdArray as $deleteId){   
                ...
                $whereCondtion  = array('id'=>$deleteId, 'role_id'=>5 );
                $this->Common_model->deleteRow('Users',$whereCondtion);  
            }
           
            $this->messages->setMessage($this->lang->line('subadmin_deleted_successfully'),'success');
            redirect(base_url('admin/subadmin'));
        }else{
