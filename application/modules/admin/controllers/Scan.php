<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    */  
    ...
    public function index($filter=''){ 
        ...
        if($this->roleId==1 || ($this->userListPer==1)){
            $this->breadcrumbs->push($this->lang->line('scan_list'), '/');
        }else{
        ...
        $data['title']=$this->lang->line('scan_list');
        
        if($this->roleId==1 || ($this->userListPer==1)){
            $this->home_template->load('home_template','admin/scan/scan_meterials_list', $data); 
        }else{
    ...
    *  @return: json_obj
    */ 
    public function scanmeterialsajaxlist(){
        ...
        $totalRecord      = $this->Scan_model->scanmeterialsajaxlist(true);
        $getRecordListing = $this->Scan_model->scanmeterialsajaxlist(false,$start,$length, $column_name, $order[0]['dir']);
        $recordListing = array();
        $content='[';
        $i=0;		
        $srNumber=$start;		
        if(!empty($getRecordListing)) {
            ...
            foreach($getRecordListing as $recordData) {
                ...
                $recordListing[$i][2]=  $recordData->country_id."".$recordData->mobile_no;
				//$recordListing[$i][3]=  $recordData->material_name;
                 $recordListing[$i][3]= (!empty($recordData->runningDepositNumber)) ? $recordData->runningDepositNumber : "-";
				...
				$recordListing[$i][6]= change_date_formate($recordData->created);
