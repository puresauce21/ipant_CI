...
class Users extends REST_Controller {
	...
	public function __construct() {
		...
		header('Content-Type: application/json');
                log_message('info','1111111111111111111111111111');
		$this->load->library('form_validation');
                
                log_message('info','22222222222222222222');
				...
				{ 
					...
					{ 
						...
						if($result) 
						{
			                ...
			                $wheres = array("user_id" => $user_id,'otp_type' => $this->input->post('varification_type'));
				        ...
				        if($user_id)
				        {
				        	...
				        	$data_val[]        = array('user_id'=>"$user_id",'otp'=>$otpnumber,'varification_type'=>$this->input->post('varification_type'));
				        	$arg['status']     = 1;
			...
			else
			{
				...
				$this->form_validation->set_rules('email', 'Email', 'valid_email' , array( 
					'valid_email' => $this->lang->line('email_valid')
                                ...
                                $this->form_validation->set_rules('country_code', 'Country Code', 'required', array( 'required' => $this->lang->line('country_code')));//Changes by harish
				$this->form_validation->set_rules('mobile_no', 'Mobile', 'required|min_length[9]|max_length[12]|numeric', array(
						'required' => $this->lang->line('mobile_required'),
						'min_length' => $this->lang->line('mobile_min_length'),
						'max_length' => $this->lang->line('mobile_max_length'),
						'numeric' => $this->lang->line('mobile_numeric')
				...
				else
				{
					...
					$email           = $this->input->post('email');
                                        $country_code = $this->input->post('country_code');//Changes by harish
					$mobile_no       = $this->input->post('mobile_no');
				    ...
				    else
				    {
						...
						if(empty($email_exist))
						{
							...
							$userdata = array(
											    ...
											    'email'      	=> $email,
											    'mobile_no'  	=> $mobile_no,
											    'country_code'  	=> $country_code,//Changes by harish
											    'qr_code'    	=> $qr_number,
					...
					$condition = array(
								...
								'otp' => $otp,
						       	'otp_type' => $varification_type
                                        ...
                                        log_message('info',print_r($condition,TRUE));
					$result = getdatafromtable($this->db->dbprefix.'Users_Otp_Verification', $condition);
					if($result)
					{
						$condition = array(
...
//                                $this->form_validation->set_rules('country_code', 'Country Code', 'required', array( 'required' => $this->lang->line('country_code')));//Changes by harish
                                ...
                                //max_length changed by harish
				$this->form_validation->set_rules('mobile_no', 'Mobile', 'required|min_length[9]|max_length[15]|numeric', array(
					'required' => $this->lang->line('mobile_required'),
					'min_length' => $this->lang->line('mobile_min_length'),
					'max_length' => $this->lang->line('mobile_max_length'),
					'numeric' => $this->lang->line('mobile_numeric')
                                ...
                                log_message('info','--------------------------------');
                                log_message('info','mobile number=========='.$this->input->post('mobile_no'));
				...
				else
				{
					...
					{       //issue identified by harish and is fixed
						$otpid = $this->dynamic_model->insertdata($this->db->dbprefix.'Users_Otp_Verification', $otpdetail);                                                
