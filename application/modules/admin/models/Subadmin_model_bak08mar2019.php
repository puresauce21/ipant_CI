...
class Subadmin_model extends CI_model{
	...
	public function subadminajaxlist($isCount=false,$start=0,$stop=0, $column_name='',$order='') {
        ...
        if(!empty($search['value'])){
           ...
           $this->db->where('(CONCAT(Users.firstname," ",Users.lastname) LIKE "%'.$search_info.'%" OR  `Users.email` LIKE "%'.$search_info.'%" OR `Users.mobile_no` LIKE "%'.$search_info.'%" OR `Roles.role_name` LIKE "%'.$search_info.'%")',NUll);
